...
(async()=>{
 ...
 for(const width of [1440,375]){
  ...
  await page.locator('[data-copy-target="practice-s2"]').click();
  const copy=await page.evaluate(()=>window.__copied);assert.match(copy,/План — 12 участников/);assert.match(copy,/Подготовь черновик делового письма/);
  await page.locator('[data-copy-target="practice-s3"]').click();
  const safeCopy=await page.evaluate(()=>window.__copied);assert.doesNotMatch(safeCopy,/12 840 000|example.invalid|Учебный А/);assert.match(safeCopy,/5 дней/);
  assert.equal(await page.locator('pre[id^="prompt-template-"]').count(),5);
  assert.equal(await page.evaluate(()=>{const ids=[...document.querySelectorAll('[id]')].map(x=>x.id);return ids.length===new Set(ids).size}),true);
  ...
  await mini.locator('[value="name"]').uncheck();await mini.locator('[value="goal"]').check();await mini.locator('[value="delay"]').check();await mini.locator("[data-minimize-check]").click();assert.match(await mini.locator(".feedback.ok").innerText(),/срок|задержк|цель/i);
